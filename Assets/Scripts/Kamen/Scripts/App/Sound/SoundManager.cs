...
namespace Kamen
{
    public class SoundManager : SingletonComponent<SoundManager>
    {
        ...
        {
            //IsSoundEffectsOn = DataSaveManager.Instance.MyData.IsSoundOn;
            //IsMusicOn = DataSaveManager.Instance.MyData.IsSoundOn;

            Play("Music first part");
            StartCoroutine(WaitToStartSecondPart());
        }
        public void Play(string id, bool isLoop = false, float playDelay = 0)
        {
            ...
            if ((soundInfo.Type == SoundType.Music && !IsMusicOn) || (soundInfo.Type == SoundType.SoundEffect && !IsSoundEffectsOn))
            {
                _volume = 0;
            }
            else _volume = 0.3f;
        ...
        private IEnumerator WaitToStartSecondPart()
        {
            AudioSource audioSource = PlayingAudioSources[0].Source;
            while (audioSource.isPlaying)
            {
                yield return null;
            }
            Destroy(audioSource.gameObject);
            PlayingAudioSources.RemoveAt(0);
            Play("Music second part", true);
        }
        ...
        public void Stop(SoundType type)
        {
            ...
            if (PlayingAudioSources.Count > 0) PlayingAudioSources[0].Source.volume = 0;
        }
